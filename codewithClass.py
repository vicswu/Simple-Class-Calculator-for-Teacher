    ...
    def update_grade (self, which_grade,new_grade):
        ...
        if which_grade == 'T2':
            if int(new_grade)<21 and int(new_grade)>(-1):    #force user to enter a grade on 20 for t2
                ...
                print("\nYou must enter a valid test grade")
        if which_grade == 'A1':
            if int(new_grade)<16 and int(new_grade)>(-1):
...
#Option 2        
        ...
        while option:
            if students_list==[]:
                ...
                print("\nName | ID | Total grade | Letter grade | T1 grade | T2 grade | A1 grade | A2 grade | A3 grade | A4 grade")
            ...
            if student_update=="done":
                ...
                for s in students_list: #for loop to search all objects in list
                    if (s.name == naminput) and (s.id == idinput): #check for which index name and id match
                        ...
                        s.update_grade(which_grade, int(new_grade))
                        break
                if (which_grade!="T1") and (which_grade!="T2") and (which_grade!="A1") and (which_grade!="A2") and (which_grade!="A3") and (which_grade!="A4"):
...
#Option 5            
        ...
        while option:
            if students_list == []: #check if list empty
                    ...
                    break
            letter_grade = input("\nEnter a letter grade (A,B,C,F) or done: ") #ask user to input their search
